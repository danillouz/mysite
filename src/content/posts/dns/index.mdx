...
<figure>
  <Image
    ...
    alt="The labels of the domain names github.com, www.cloudflare.com and en.wikipedia.org visualized in the domain name space."
  />
...
- In the domain name `bbc.co.uk`:
  - `uk` is the TLD (ccTLD).
  - `co` is the 2LD.
  - `bbc` is the 3LD.
- In the domain name `www.framer.com`:
  - `com` is the TLD (gTLD).
  - `framer` is the 2LD.
  - `www` is the 3LD (will usually just be called a subdomain).
