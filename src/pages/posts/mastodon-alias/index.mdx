...
For example, to get WebFinger information for the Mastodon account `danillouz@mastodon.social`[^2], you need to make an HTTP `GET` request to:
...
The RFC [mentions](https://www.rfc-editor.org/rfc/rfc7033#section-1) that WebFinger information is static:
...
I'm using [Astro](https://astro.build/), so I just added a [static file endpoint](https://docs.astro.build/en/core-concepts/endpoints/#static-file-endpoints) by creating the file:
...
Note that the [static JSON endpoint](#static-file-endpoint) I added will only serve the WebFinger information when making an HTTP `GET` request to:
...
I'm using [Vercel](https://vercel.com/), which supports [redirects](https://vercel.com/docs/project-configuration#project-configuration/redirects) by creating a `vercel.json` file. So I can achieve the desired redirect by adding the following rule:
...
And the docs [mention](https://docs.joinmastodon.org/user/signup/#address) that you should include the server domain when sharing your handle with other people, because otherwise they won't be able to find you easily:
...
And checking the v2 search API [docs](https://docs.joinmastodon.org/methods/search/#query-parameters), we can see that `resolve` controls if a WebFinger lookup should happen or not:
...
I think the latter post is pretty cool, where Lindsay is also fetching Mastodon posts via RSS to show them on their site. Turns out you can postfix any account with `.rss`, and Mastodon will give you the RSS feed for it. This reminded me of the Reddit API. So I tried `.json`, and that also works! For example:

- [https://mastodon.social/@danillouz.rss](https://mastodon.social/@danillouz.rss)
- [https://mastodon.social/@danillouz.json](https://mastodon.social/@danillouz.json)
