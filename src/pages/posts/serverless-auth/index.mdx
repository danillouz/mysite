...
And therefore part of the `verifiedData`, so we can propagate it to downstream Lambda handlers like this:
...
In order to upload the Lambda to AWS, [sign up](https://portal.aws.amazon.com/billing/signup) and make sure you have your [credentials configured](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-files.html). Then release the Lambda with:
...
<figure>
  <Image
    ...
    alt="Image that shows where to find the Lambda Authorizer ARN in the AWS Lambda Console."
  />
