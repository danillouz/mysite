...
description: "How to transcode short audio files with Lambda and Amazon Elastic Transcoder or FFmpeg."
tags:
  [
    "audio",
    "aws",
...
The layer is named `ffmpeg` and the `path` property dictates that the layer code will reside in a directory named `layers`. Match this structure in the project:

```sh
mkdir layers
```

Move into this directory and download a static build of FFmpeg from [johnvansickle.com/ffmpeg](https://johnvansickle.com/ffmpeg)[^5].
...
This module will spawn a [synchronous child process](https://nodejs.org/api/child_process.html#child_process_child_process_spawnsync_command_args_options) that allows us to execute the `ffmpeg` "command" (provided by the FFmpeg layer):
...
The logs tell us that FFmpeg is executing (hooray!) but that it doesn't complete (booo!).
...
>
> <cite>[Lambda pricing page](https://aws.amazon.com/lambda/pricing)</cite>
